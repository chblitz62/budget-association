      ...
      <div className={`${card} p-6`}>
        ...
        <div className={`mt-5 grid grid-cols-1 md:grid-cols-3 gap-4 pt-5 border-t ${dm ? 'border-gray-700' : 'border-slate-200'}`}>
          <div className={`p-4 rounded-2xl ${dm ? 'bg-blue-900/30' : 'bg-blue-50'}`}>
            <div className={`text-xs font-bold uppercase mb-1 flex items-center gap-1 ${dm ? 'text-blue-400' : 'text-blue-600'}`}>
              Total Frais Fixes <HelpIcon {...H.fraisFixes} darkMode={dm} position="top" />
            </div>
          ...
          <div className={`p-4 rounded-2xl ${dm ? 'bg-indigo-900/30' : 'bg-indigo-50'}`}>
            <div className={`text-xs font-bold uppercase mb-1 flex items-center gap-1 ${dm ? 'text-indigo-400' : 'text-indigo-600'}`}>
              Heures vendues / an <HelpIcon {...H.heuresVendues} darkMode={dm} position="top" />
            </div>
          ...
          <div className={`p-4 rounded-2xl ${dm ? 'bg-violet-900/30' : 'bg-violet-50'}`}>
            <div className={`text-xs font-bold uppercase mb-1 flex items-center gap-1 ${dm ? 'text-violet-400' : 'text-violet-600'}`}>
              Taux Structure / heure <HelpIcon {...H.tauxStructure} darkMode={dm} position="top" />
            </div>
                  ...
                  {isVac ? (
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <div className="flex items-center gap-1">
                        <span className={`flex items-center gap-0.5 ${textMuted}`}>Sal. brut/mois <HelpIcon {...H.salaireBrutPilotage} darkMode={dm} position="bottom" /></span>
                        <input type="number" value={s.salaireBrut}
                          onChange={e => upSal(i, 'salaireBrut', parseFloat(e.target.value) || 0)}
                          className={`w-20 ${inputSmCls} text-right`} />
                      </div>
                      <div className="flex items-center gap-1">
                        <span className={`flex items-center gap-0.5 ${textMuted}`}>Ch.% <HelpIcon {...H.tauxChargesPilotage} darkMode={dm} position="bottom" /></span>
                      ...
                      <div className="flex items-center gap-1">
                        <span className={`flex items-center gap-0.5 ${textMuted}`}>H/sem <HelpIcon {...H.heuresHebdo} darkMode={dm} position="bottom" /></span>
                        <input type="number" value={s.heuresHebdo}
                          onChange={e => upSal(i, 'heuresHebdo', parseFloat(e.target.value) || 0)}
                          className={`w-14 ${inputSmCls} text-right`} />
                      </div>
                      <div className="flex items-center gap-1">
                        <span className={`flex items-center gap-0.5 ${dm ? 'text-amber-400' : 'text-amber-600'} font-bold`}>H.hors-prod/sem <HelpIcon {...H.heuresHorsProd} darkMode={dm} position="bottom" /></span>
                      ...
                      <div className="flex items-center gap-1">
                        <span className={`flex items-center gap-0.5 ${dm ? 'text-amber-400' : 'text-amber-600'} font-bold`}>Ratio prépa <HelpIcon {...H.ratioPreparation} darkMode={dm} position="bottom" /></span>
                      ...
                      <div className="flex items-center gap-1">
                        <span className={`flex items-center gap-0.5 ${dm ? 'text-red-400' : 'text-red-600'} font-bold`}>Abs. (j/an) <HelpIcon {...H.joursAbsencePilotage} darkMode={dm} position="bottom" /></span>
                  ...
                  <div className="ml-auto flex items-center gap-3">
                    {!isVac && (
                      <div className="text-right">
                        <div className={`text-xs line-through flex items-center justify-end gap-0.5 ${dm ? 'text-gray-500' : 'text-slate-400'}`}>
                          {c.coutHoraireContractuel.toFixed(2)} €/h
                          <HelpIcon {...H.coutHoraireContractuel} darkMode={dm} position="left" />
                        </div>
                        <div className={`text-xs ${dm ? 'text-orange-400' : 'text-orange-600'}`}>+{c.ecartCoutHoraire.toFixed(2)} €</div>
                      </div>
                    )}
                    <div className={`flex items-center gap-1 px-3 py-1.5 rounded-xl font-black text-sm ${
                      isVac
                      ...
                      <HelpIcon {...H.coutHoraireFacture} darkMode={dm} position="left" />
                    </div>
                    {salaries.length > 1 && (
                  ...
                  <div className={`p-3 rounded-xl ${dm ? 'bg-gray-700' : 'bg-white'} space-y-1`}>
                    <div className="flex justify-between text-sm">
                      <span className={`flex items-center gap-1 ${textMuted}`}>
                        Coût formateur ({activeSess.nbHeures}h)
                        <HelpIcon {...H.coutHoraireFacture} darkMode={dm} position="left" />
                      </span>
                    ...
                    <div className="flex justify-between text-sm">
                      <span className={`flex items-center gap-1 ${textMuted}`}>
                        Frais structure
                        <HelpIcon {...H.fraisStructureSession} darkMode={dm} position="left" />
                      </span>
                  ...
                  <div className={`p-4 rounded-2xl border ${
                    ...
                    <div className="flex justify-between items-center mb-1">
                      <span className={`font-black flex items-center gap-1 ${activeRes.marge >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                        ...
                        Marge nette
                        <HelpIcon {...H.marge} darkMode={dm} position="top" />
                      </span>
                    ...
                    <div className={`text-xs grid grid-cols-2 gap-2 ${textMuted}`}>
                      <div className="flex items-center gap-1">
                        Taux de marge :
                        <HelpIcon {...H.tauxMargeSession} darkMode={dm} position="top" />
                        <strong className={activeRes.marge >= 0 ? 'text-green-500' : 'text-red-500'}>{activeRes.tauxM.toFixed(1)}%</strong>
                      </div>
                      <div className="flex items-center gap-1">
                        Point mort :
                        <HelpIcon {...H.pointMort} darkMode={dm} position="top" />
                        <strong className={textPri}>{isFinite(activeRes.pm) ? `${activeRes.pm} stag.` : '—'}</strong>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <div className="space-y-6">
      ...
      <div className={`${card} p-6`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
          {[
            { l: 'Frais Fixes',    v: `${Math.round(totalFF).toLocaleString()} €`, bg: dm ? 'bg-blue-900/30'   : 'bg-blue-50',   c: dm ? 'text-blue-400'   : 'text-blue-600',   help: H.fraisFixes    },
            { l: 'Heures vendues', v: totalHV.toLocaleString(),                     bg: dm ? 'bg-indigo-900/30' : 'bg-indigo-50', c: dm ? 'text-indigo-400' : 'text-indigo-600', help: H.heuresVendues  },
            { l: 'Taux structure', v: `${tauxConsolide.toFixed(2)} €/h`,            bg: dm ? 'bg-violet-900/30' : 'bg-violet-50', c: dm ? 'text-violet-400' : 'text-violet-600', help: H.tauxStructure  },
            { l: 'Formateurs',     v: totalForm,                                    bg: dm ? 'bg-teal-900/30'   : 'bg-teal-50',   c: dm ? 'text-teal-400'   : 'text-teal-600',   help: null             },
            { l: 'Sessions',       v: totalSessions,                                bg: dm ? 'bg-orange-900/30' : 'bg-orange-50', c: dm ? 'text-orange-400' : 'text-orange-600', help: null             },
            { l: 'CA total',       v: `${Math.round(totalRec).toLocaleString()} €`, bg: dm ? 'bg-green-900/30'  : 'bg-green-50',  c: dm ? 'text-green-400'  : 'text-green-600',  help: null             },
            ...
            <div key={k.l} className={`p-4 rounded-2xl ${k.bg}`}>
              <div className={`text-xs font-bold uppercase mb-1 flex items-center gap-1 ${k.c}`}>
                {k.l}
                {k.help && <HelpIcon {...k.help} darkMode={dm} position="bottom" />}
              </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={`text-xs uppercase font-bold ${textMuted} border-b ${dm ? 'border-gray-700' : 'border-slate-200'}`}>
                ...
                <th className="text-right pb-3 px-3"><span className="flex items-center justify-end gap-1">Frais Fixes <HelpIcon {...H.fraisFixes} darkMode={dm} position="bottom" /></span></th>
                <th className="text-right pb-3 px-3"><span className="flex items-center justify-end gap-1">H. vendues <HelpIcon {...H.heuresVendues} darkMode={dm} position="bottom" /></span></th>
                <th className="text-right pb-3 px-3"><span className="flex items-center justify-end gap-1">Taux structure <HelpIcon {...H.tauxStructure} darkMode={dm} position="bottom" /></span></th>
                <th className="text-right pb-3 px-3">Formateurs</th>
                <th className="text-right pb-3 px-3">Sessions</th>
                <th className="text-right pb-3 px-3"><span className="flex items-center justify-end gap-1">Tx. occup. <HelpIcon {...H.tauxOccupation} darkMode={dm} position="bottom" /></span></th>
                <th className="text-right pb-3 px-3">CA sessions</th>
                <th className="text-right pb-3 px-3"><span className="flex items-center justify-end gap-1">Marge <HelpIcon {...H.marge} darkMode={dm} position="bottom" /></span></th>
              </tr>
            </thead>
            <tbody>
              {stats.map(s => (
...
const PilotageFinancier = ({ darkMode: dm, checkPassword, budgetPersonnel = [], services: servicesProp, externalSites, setExternalSites }) => {
  ...
  const [activeTab, setActiveTab]   = useState(() => {
    const first = externalSites?.[0] ?? zeroSites[0];
  ...
  const addSite = () => {
    const newId = Date.now();
    const newSite = { id: newId, nom: `Nouveau Site ${sites.length + 1}`, fraisFixes: [], heuresVendues: 0, salaries: [], sessions: [] };
