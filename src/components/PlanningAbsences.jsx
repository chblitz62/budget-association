...
const PlanningAbsences = ({ direction, poleSupport, services, planningAbsences, setPlanningAbsences, darkMode }) => {
  ...
  const getAbsence = (agentId, source, day) => {
    const dayKey = `${year}-${padNum(month + 1)}-${padNum(day)}`;
    const agentKey = `${agentId}-${source}`;
    return planningAbsences?.[monthKey]?.[agentKey]?.[dayKey] ?? null;
  };

  const cycleAbsence = (agentId, source, day) => {
    const dayKey = `${year}-${padNum(month + 1)}-${padNum(day)}`;
    const agentKey = `${agentId}-${source}`;
    const current = getAbsence(agentId, source, day);
  ...
  const totalAbsencesForAgent = (agentId, source) => {
    const agentKey = `${agentId}-${source}`;
    const monthData = planningAbsences?.[monthKey]?.[agentKey] || {};
    return Object.values(monthData).filter(v => v !== null).length;
  };

  const totalAbsencesForDay = (day) => {
    return agents.reduce((sum, a) => {
      return getAbsence(a.id, a.source, day) !== null ? sum + 1 : sum;
    }, 0);
  };
  ...
  const totalJoursAbsence = agents.reduce((sum, a) => sum + totalAbsencesForAgent(a.id, a.source), 0);
    ...
    <div id="planning-absences" className={`rounded-3xl shadow-lg border-2 p-6 mb-8 print-avoid-break ${darkMode ? 'bg-gray-800 border-violet-900' : 'bg-gradient-to-br from-violet-50 to-purple-50 border-violet-200'}`}>
      ...
      {agents.length === 0 ? (
        <p className={`text-center py-8 text-sm ${darkMode ? 'text-gray-500' : 'text-slate-400'}`}>
          Aucun agent renseigné — ajoutez du personnel dans les sections Direction, Pôle Support ou Services.
        </p>
      ) : (
          ...
          <table className="text-xs border-collapse min-w-full">
            <thead>
              <tr>
                ...
                {days.map(d => (
                  ...
                  >
                    <div>{d}</div>
                    <div className={`text-[9px] font-normal ${darkMode ? 'text-gray-600' : 'text-slate-400'}`}>{JOURS_COURTS[getDayOfWeek(d)]}</div>
                  </th>
                ))}
            ...
            <tbody>
              {agents.map((agent, aIdx) => {
                const totalAgent = totalAbsencesForAgent(agent.id, agent.source);
                return (
                  <tr key={`${agent.id}-${agent.source}`} className={`${aIdx % 2 === 0 ? (darkMode ? 'bg-gray-800/50' : 'bg-white/60') : (darkMode ? 'bg-gray-700/30' : 'bg-violet-50/40')}`}>
                    <td className={`px-2 py-1 sticky left-0 z-10 ${aIdx % 2 === 0 ? (darkMode ? 'bg-gray-800' : 'bg-white') : (darkMode ? 'bg-gray-750 bg-gray-700' : 'bg-violet-50')}`}>
                      ...
                      <div className={`text-[10px] truncate max-w-[150px] ${darkMode ? 'text-gray-500' : 'text-slate-400'}`}>{agent.source}</div>
                    </td>
                    {days.map(d => {
                      const absence = getAbsence(agent.id, agent.source, d);
                        ...
                        <td key={d} className={`p-0.5 text-center ${weekend ? (darkMode ? 'bg-gray-700/40' : 'bg-slate-100/60') : ''}`}>
                          {weekend ? (
                            <div className={`w-7 h-7 rounded flex items-center justify-center mx-auto ${darkMode ? 'bg-gray-700/40' : 'bg-slate-100'}`}>
                              <span className={`text-[9px] ${darkMode ? 'text-gray-600' : 'text-slate-300'}`}>—</span>
                            </div>
                          ) : (
                            <button
                              onClick={() => cycleAbsence(agent.id, agent.source, d)}
                              ...
                              title={absence ? LABELS[absence] : 'Cliquer pour ajouter une absence'}
                            >
                              {absence && (
                                <span className={`text-[9px] font-black ${TEXT_CLASSES[absence]}`}>
                ...
                {days.map(d => {
                  ...
                  return (
                    <td key={d} className={`px-1 py-1.5 text-center font-black ${weekend ? (darkMode ? 'bg-gray-700/40 text-gray-600' : 'bg-slate-100 text-slate-300') : ''}`}>
                      {!weekend && tot > 0 ? (
                        <span className={`text-[10px] font-black px-1 rounded ${darkMode ? 'bg-violet-900/60 text-violet-300' : 'bg-violet-200 text-violet-800'}`}>
                          {tot}
                        </span>
                      ) : (!weekend ? <span className={`text-[10px] ${darkMode ? 'text-gray-600' : 'text-slate-300'}`}>—</span> : null)}
                    </td>
                  );
                })}
          ...
          {TYPES_ABSENCE.filter(t => t !== null).map(type => {
            const count = agents.reduce((sum, a) => {
              return sum + days.reduce((s2, d) => {
                ...
                return getAbsence(a.id, a.source, d) === type ? s2 + 1 : s2;
              }, 0);
            }, 0);
